...
class Renderer(object):
	...
	def glTriangle(self, A, B, C, color = None):
		# Usar el color proporcionado o el color actual
		triangle_color = color or self.currColor
		...
		def flatBottom(vA, vB, vC):
			...
			try:
				...
				for y in range(round(vB[1]), round(vA[1] + 1)):
					for x in range(round(x0), round(x1 + 1)):
						self.glPoint(x, y, triangle_color)
	...
	def glDrawPrimitives(self, buffer, vertexOffset):
		...
		if self.primitiveType == POINTS:
			...
			for i in range(0, len(buffer), vertexOffset * 3):
				...
				# Generar un color fijo para cada triángulo basado en su índice
				# Usar el índice del triángulo como semilla para generar colores consistentes
				random.seed(triangle_index)
				random_color = [random.random(), random.random(), random.random()]

				self.glTriangle(A, B, C, random_color)
				triangle_index += 1
