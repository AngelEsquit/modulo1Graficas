...
class Renderer(object):
    ...
    def glTriangle(self, A, B, C, uv_coords=None, vertex_colors=None):
        """
        Rasteriza un triángulo con soporte para colores por vértice (rainbow) y fragment shaders.
        """
        ...
        for x in range(minX, maxX + 1):
            for y in range(minY, maxY + 1):
                ...
                if w0 >= 0 and w1 >= 0 and w2 >= 0:
                    ...
                    elif uv_coords and self.activeTexture:
                        # Textura normal (código existente)
    ...
    def _process_model(self, model):
        """Procesa un modelo con soporte para rainbow colors y fragment shaders."""
        ...
        # NUEVO: Pasar los colores a glDrawPrimitives
        self.glDrawPrimitives(processed_vertices, 3, uv_coords if has_uv else None, vertex_colors if vertex_colors else None)


    def glDrawPrimitives(self, buffer, vertexOffset, uvBuffer=None, colorBuffer=None):
        """Dibuja primitivas con soporte para colores por vértice."""
        if self.primitiveType == TRIANGLES:
            for i in range(0, len(buffer), vertexOffset * 3):
                ...
                # NUEVO: Extraer colores para este triángulo
                color_coords_triangle = None
                if colorBuffer and len(colorBuffer) > i//vertexOffset + 2:
                    color_coords_triangle = [colorBuffer[j] for j in range(i//vertexOffset, i//vertexOffset + 3)]

                self.glTriangle(A, B, C, uv_coords_triangle, color_coords_triangle)
